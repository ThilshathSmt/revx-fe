...
const ManagerNotificationPage = ({ isPopup, anchorEl, onClose }) => {
  const { user } = useAuth();
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      setUnreadCount(managerNotifications.filter((n) => !n.isRead).length);
    } catch (error) {
  ...
  const handleOpenViewDialog = async (notification) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/goalReviews/${notification.relatedEntityId}`,
        { headers: { Authorization: `Bearer ${user.token}` } }
      );
      ...
      >
        <Box>
          <Box sx={{ 
            ...
            <Typography variant="h6">Manager Notifications</Typography>
            {unreadCount > 0 && (
        ...
        >
          ...
          >
            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
              ...
              <Typography variant="h6">Goal Review Details</Typography>
            </Box>
          ...
          <DialogContent sx={{ mt: 2 }}>
            {isLoadingReview ? (
              ...
              <Grid container spacing={3} sx={{ mt: 1 }}>
                ...
                <Grid item xs={12} sm={6}>
                  <Typography variant="subtitle1"><strong>Team:</strong> {selectedReview?.team?.teamName || "N/A"}</Typography>
                </Grid>
                <Grid item xs={12} sm={6}>
