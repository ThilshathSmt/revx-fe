...
const EmployeeNotificationPage = ({ isPopup, anchorEl, onClose }) => {
  const { user } = useAuth();
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      // Filter for employee-specific task review notifications only
      const employeeNotifications = response.data.filter(
        n => n.type === "TaskReviewCreated"
      );
      setNotifications(employeeNotifications);
      setUnreadCount(employeeNotifications.filter((n) => !n.isRead).length);
    } catch (error) {
  ...
  const handleOpenViewDialog = async (notification) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/taskReviews/view/${notification.relatedEntityId}`,
        { headers: { Authorization: `Bearer ${user.token}` } }
      );
      ...
      >
        <Box>
          <Box sx={{ 
            ...
            <Typography variant="h6">Task Notifications</Typography>
            {unreadCount > 0 && (
          ...
          <Box sx={{ maxHeight: 400, overflow: "auto" }}>
            {recentNotifications.length > 0 ? (
              ...
              <Typography sx={{ p: 3, textAlign: "center" }}>No new notifications</Typography>
            )}
          </Box>
          <Divider />
        ...
        >
          ...
          >
            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
              ...
              <Typography variant="h6">Task Review Details</Typography>
            </Box>
